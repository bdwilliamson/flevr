...
methods include the lasso {[}\textbf{tibshirani1996?}{]}, implemented in
the R package \texttt{glmnet} {[}11{]}; stability selection {[}12{]},
implemented in the R package \texttt{stabs}
{[}{[}13{]};Hofner:pkg\_stabs:2021{]}; and knockoffs {[}14{]},
implemented in the R package \texttt{knockoff} {[}15{]}. However, prior
to our package \texttt{flevr} (described below), only one R package
available on CRAN, \texttt{miselect} {[}16{]}, explicitly mentions
missing data in its title. This package forces selection of the same
variables across multiply-imputed datasets {[}17{]}; while potentially
useful in some contexts, this may be undesirable in other contexts.
Additionally, using a single algorithm to perform variable selection
carries the risk that if the underlying model assumptions are not met
(e.g., a linear regression model for the lasso), the selected variables
might not be correct. Approaches based on ensembles of multiple
algorithms may be more robust to this misspecification; examples include
random forests {[}18{]}, majority vote over the ensemble {[}19{]}, and
rank-based weighting {[}20{]}, but there is no \texttt{R} implementation
of these approaches on CRAN.

The \texttt{flevr} package for \texttt{R} {[}21{]} provides researchers
...
we implement is to use the Super Learner {[}22{]} to perform variable
...
{[}22{]}, improving robustness of selected variables to model
misspecification. The second approach is to base variable selection on
\emph{intrinsic variable importance} {[}23,24{]}, which quantifies the
...
importance {[}25{]}; this approach outputs a single set of selected
...
{[}25{]}; add math supporting this method
...
We recommend using the Super Learner {[}22{]} to do extrinsic variable
...
{[}23,24{]}. Intrinsic selection {[}25{]} also uses the Super Learner
...
Next, we use Rubin's rules {[}26{]} to combine the variable importance
...
pages and vignettes, and on the \texttt{pkgdown} website {[}27{]} at

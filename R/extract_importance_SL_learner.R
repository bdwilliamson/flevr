...
extract_importance_SL_learner <- function(fit, coef, feature_names) {
  ...
  if (any(grepl("xgb", cls))) {
    imp_dt <- extract_importance_xgboost(fit, feature_names, coef)
  } else if (any(grepl("ranger", cls))) {
    imp_dt <- extract_importance_ranger(fit, feature_names, coef)
  } else if (any(grepl("glmnet", cls))) {
    imp_dt <- extract_importance_glmnet(fit, feature_names, coef)
  } else if (any(grepl("glm", cls))) {
    imp_dt <- extract_importance_glm(fit, feature_names, coef)
  } else if (any(grepl("mean", cls)) | any(grepl("numeric", cls))) {
    imp_dt <- extract_importance_mean(fit, feature_names, coef)
  } else {
